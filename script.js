...
const categoryCountsOutput = document.getElementById('categoryCountsOutput'); // Get reference to the new element for counts
...
// --- Determine Suggestion ---
    ...
    const surroundingNumbersList = parseSurroundingNumbersString(surroundingString); // Parse into a list

    if (surroundingNumbersList.length > 0) {
        // Sort the list numerically, treating "00" as 0 for sorting
        surroundingNumbersList.sort((a, b) => {
            const numA = (a === "00") ? 0 : a;
            const numB = (b === "00") ? 0 : b;
            return numA - numB;
        });

        // Format the sorted list into a string for display
        const sortedOutputString = surroundingNumbersList.map(num => {
            return (num === "00") ? "00" : String(num);
        }).join(' | '); // Join with ' | ' separator

        // Display the sorted list in the surrounding numbers output area
        surroundingNumbersOutput.textContent = sortedOutputString;

    } else {
        // If the list is empty (e.g., error in surroundingString), clear the output
        surroundingNumbersOutput.textContent = ""; // Or display an error/N/A message if preferred
    }

    // --- Count Categories in Surrounding Numbers --- // Section heading adjusted
    // The category counting logic uses the parsed surroundingNumbersList from above.
    const categoryCounts = countCategoriesInList(surroundingNumbersList); // Use the parsed list

    // Format and display category counts
    let categoryCountsText = "Counts in Surrounding Numbers:\n";
    categoryCountsText += `1st Dozen (1-12): ${categoryCounts.firstDozen}\n`;
    categoryCountsText += `2nd Dozen (13-24): ${categoryCounts.secondDozen}\n`;
    categoryCountsText += `3rd Dozen (25-36): ${categoryCounts.thirdDozen}\n`;
    categoryCountsText += `Low (1-18): ${categoryCounts.lowNumbers}\n`;
    categoryCountsText += `High (19-36): ${categoryCounts.highNumbers}`;

    categoryCountsOutput.textContent = categoryCountsText;


    // --- Find & Display Possible Corner Bets in Surrounding Numbers --- // Section heading adjusted
    // The corner bet finding logic uses the parsed surroundingNumbersList from above.
